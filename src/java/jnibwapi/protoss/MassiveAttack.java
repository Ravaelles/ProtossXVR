...
public class MassiveAttack {

	private static XVR xvr = XVR.getInstance();

	private static final int MINIMUM_ARMY_TO_PUSH = 22;
	...
	private static boolean decideIfWeAreReadyToAttack(boolean forceMinimum) {
		...
		boolean weAreReadyToAttack = (battleUnits >= (forceMinimum ? minimumArmyToPush
				...
				|| (MapExploration.getNumberOfKnownEnemyBases() > 0 && (battleUnits >= (1.5 * MINIMUM_ARMY_TO_PUSH)
	...
	private static void decisionWhenAttacking() {
		...
		if (isNewAttackState()) {
			...
			MassiveAttack.defineInitialAttackTarget();
			// }
		}
	...
	private static void defineNextTarget() {
		...
		Unit target = TargetHandling
				.getImportantEnemyUnitTargetIfPossibleFor(getArmyCenterPoint());
		ArrayList<Unit> enemyBuildings = xvr.getEnemyBuildings();
...
//		System.out.println("_attackTargetUnit = " + _attackTargetUnit.toStringShort());
//		System.out.println("_attackPoint = " + _attackPoint);
//		System.out.println("isProperTarget(target) = " + TargetHandling.isProperTarget(target));
//		System.out.println();
	}
	...
	private static Point getArmyCenterPoint() {
		int totalX = 0;
		int totalY = 0;
		int counter = 0;
		...
		return new Point((int) ((double) totalX / counter),
