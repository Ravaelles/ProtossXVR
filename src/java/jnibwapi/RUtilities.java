...
public class RUtilities {
    ...
    /** Displays small window with <b>text</b> information. Very useful for testing, 
     * error reporting. */
    public static void displayMessage(String text) {
        JOptionPane.showMessageDialog(new JOptionPane(), text, "", JOptionPane.PLAIN_MESSAGE);
    }

    /** Displays small window with <b>text</b> information and with <b>title</b> title.
     Very useful for testing, error reporting. */
    ...
    public static void displayError(String title, String errorText) {
        JOptionPane.showMessageDialog(new JOptionPane(),
            errorText, title, JOptionPane.ERROR_MESSAGE);
    }
...
//        }
    ...
    /**
     * @param percentChance is chance percentage of some action, e.g. 87.2 means some event
     * occurs with 87.2% probability
    ...
    /**
     * @return String representing a number with given decimal places e.g. (3.1415, 2) 
     * will give "3.14"
    ...
    public static void displayException(Exception e, String title, String preText) {
        RUtilities.displayError(title, preText + "\n\n" + e.getMessage() + "\n\n"
    ...
    /** Returns just the file name, without extension part e.g. for file "images/horse.png" it will
     return "horse". */
    ...
    /** Displays report concerning total amount of time that some task has taken, also
     * it displays average per one "task object". Number of atomic tasks is <b>totalToProcess</b>. 
     * You need to pass timeStart (System.getCurrentTimeInMillis()).
     */
    public static void displayTimeReport(long timeStart, int totalToProcess) {
        ...
        System.out.println("Processing took " + 
                ...
                + " seconds  (" + String.format("%.2f", (double) (timeEnd - timeStart) / 
    ...
    /** 
     * According to current objects processed and total to process it displays estimated time to
     * finish all tasks.
     */
    public static void displayETA(long timeStart, int alreadyProcessed, int totalToProcess) {
        double seconds = ((double) (System.currentTimeMillis() - timeStart) / 
                (1000 * alreadyProcessed));
        String eta = RUtilities.convertSecondsToDisplayableFormat
                ((int) ((totalToProcess - alreadyProcessed) * seconds));
        System.out.println("It took " + String.format("%.1f", seconds) + "s. " + 
                (int) alreadyProcessed * 100 / 
                totalToProcess   + "% objects (" + alreadyProcessed + "/" + totalToProcess 
                + ") ready. ETA: " + eta);
    }
...
//                Global.deunderlineLabel(label);
    ...
    /** Returns String value, according to the relative comparison of the provided value to the
     * possible range e.g. we can assign letters from A to Z for numbers 1-10.
     *     */
    ...
    public static <K, V extends Comparable<? super V>> Map<K, V> 
        ...
        java.util.List<Map.Entry<K, V>> list =
            new LinkedList<java.util.Map.Entry<K, V>>( map.entrySet() );
    ...
    /** Returns median of given double list.
     * @param mathematicMedian If true it will return normal median. If it is set to false and number 
     * of elements is even the center (but lesser) element will be returned e.g. for [1 2 3 4] it would 
     * return 2.
     *     */
