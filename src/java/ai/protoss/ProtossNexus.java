...
public class ProtossNexus {
	...
	private static void checkIfRemoveZeroMineralsCrystal(Unit base) {
		...
		final int ACT_IF_AT_LEAST_N_WORKERS = 28;
	...
	private static boolean shouldBuildWorkers(Unit base) {
		...
		// If we have only one base and already some workers, promote more gateways
		if (UnitCounter.getNumberOfUnits(buildingType) == 1 && UnitCounter.getNumberOfUnits(UnitTypes.Protoss_Gateway) < 3) {
